...
//V08 - 2017 06 25
//Incorpora fondo e imagen original
...
function draw(){
  background(0,0,0,80);
  insignia.actualizar();
  image(indio,insignia.centro.x,insignia.centro.y);
}
  ...
  this.cargarInsignia = function(){
    ...
    for(var yDest = 0; yDest < int(this.insigniaFuente.height/this.numPixeles); yDest++){
      for(var xDest = 0; xDest < int(this.insigniaFuente.width/this.numPixeles); xDest++){
        ...
        this.puntos[i] = new Punto(this.numPixeles*(xDest+.5), this.numPixeles*(yDest+.5), 1.4*this.numPixeles, r/norma, g/norma, b/norma, alpha/norma);
        i++;
      }
    }
  }
