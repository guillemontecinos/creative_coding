//gravitational field as an array
...
function setup(){
  ...
  vAux = createVector();
  //Create grav field
  field[0] = new Field(width/4,height/2);
  field[1] = new Field(3*width/4,height/2);
...
function Particle(x,y,radius, transp){
  ...
  this.move = function(){
    ...
    for (var i = 0; i < field.length; i++) {
      vAux.add(p5.Vector.sub(field[i].pos,this.pos));
    }
    this.direction.set(vAux); //= p5.Vector.sub(field.pos,this.pos);
    this.dirNorm=this.direction.normalize();

    //Set acceleration
    var distancia =0;// this.pos.dist(field.pos);

    for (var i = 0; i < field.length; i++) {
      distancia += this.pos.dist(field[i].pos);
    }
